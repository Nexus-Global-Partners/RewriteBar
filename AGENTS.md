...
21. Luna runs only through the signed official OpenAI Codex runtime, with an app-owned Codex home, managed ChatGPT sign-in, ephemeral threads, no extra reasoning delay, read-only sandboxing, model-action network access disabled, disabled approvals, command/image-view/web/browser/computer/app/plugin/skill/workspace-dependency/dynamic tools disabled at process startup, fail-closed unexpected events, and structured text output.
